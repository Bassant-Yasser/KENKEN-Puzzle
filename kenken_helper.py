...
class Kenken_Board:
    ...
    def change_size(self, size):
        ...
        Args:
            size (int): The new size of the puzzle.
        """
        self.size = size
    ...
    def Board_generator(self):
        ...
        for j in range(self.size):           # for each row of the board
            board.append([])            # add a new row
            for i in range(self.size):       # for each column
                board[j].append(((i + j) % self.size) + 1 )  # add a new cell to the row
                
        for _ in range(self.size):           # for each row in the board
            shuffle(board)              # shuffle the row

        for c1 in range(self.size):          # for each column in the board
            for c2 in range(self.size):      # for each column in the board
                if random() > 0.5:      # if the random number is greater than 0.5
                    for r in range(self.size):   # for each row in the board
                        board[r][c1], board[r][c2] = board[r][c2], board[r][c1] # swap the cells
        dict_board = {}                 # dictionary of (j, i) : value
        for j in range(self.size):           # for each row in the board
            for i in range(self.size):       # for each column
    ...
    def Generate_groups(self):
        ...
        dict_board, uncaged = self.Board_generator()
